...
truevalue = c(beta00,sd_beta0, AR,sd_noise,NA)

parameter = c("beta00","sd_beta0","AR","sd_noise","sd_int")

###################################################
K = length(truevalue)
npar = K
myfiles = list.files(pattern=paste0("^Result_TVP_rw_shift",shift,"_N",N,"O",O,"_.*\\.Rdata"),full.names = TRUE)
Nrep = length(myfiles)
myfiles_new = c()
for (i in 1:Nrep){
  load(myfiles[i])
...
for (i in 1:nrep){
    ...
    #load(gsub("Result_TVP_rw", "RSTVP_gcm_simulateddata", myfiles_new[i]))
    ...
    temp = resulttable[c(grep("beta00",rownames(resulttable)),
                        ...
                        grep("sd_int",rownames(resulttable))), c(1,4,5,6)]
    for(k in 1:K){
    for(j in 1:4){
...
write.csv(sumdat, file=paste0("SummaryMeasures_TVP_rw_shift",shift,"_N",N,"O",O,".csv"))
sumdat
nrep
