...
truevalue = c(beta00,sd_beta0,beta1,gamma0[2],NA,sd_gamma[2],NA,
              AR,sd_noise,
              alpha[1,2,1],alpha[1,1,2],NA,NA,NA,NA)

parameter = c("beta00", "sd_beta0", "beta1", "gamma0[2]", "gamma0[3]", "sd_gamma[2]", "sd_gamma[3]", 
              "AR", "sd_noise",
...
K = length(truevalue)
npar = K
myfiles = list.files(pattern=paste0("^Result_RSTVP_gcm_3regimes_shift",shift,"_N",N,"O",O,"_.*\\.Rdata"),full.names = TRUE)
Nrep = length(myfiles)
myfiles_new = c()
for (i in 1:Nrep){
  load(myfiles[i])
...
for(pp in 1:N){
  for(oo in 1:O){
    ...
    load(gsub("Result_RSTVP_gcm_3regimes", "RSTVP_gcm_simulateddata", myfiles_new[i]))
    ...
    # Get parameter estimates
      temp = resulttable[c(grep("beta00",rownames(resulttable)),
                         ...
                         grep("beta1",rownames(resulttable)),
...
write.csv(sumdat, file=paste0("SummaryMeasures_RSTVP_gcm_3regimes_shift",shift,"_N",N,"O",O,".csv"))
sumdat
nrep
